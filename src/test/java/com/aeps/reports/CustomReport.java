...
public class CustomReport implements IReporter {
			...
			for (ClassResult classResult : classResults) {
				...
				for (MethodResult methodResult : classResult.getMethodResults()) {
					...
					for (ITestResult result : results) {
						 ...
						 * int i = 1; i < resultsCount; i++ ) { buffer.append(
						 * "<tr class=\"").append(cssClass)
						 * .append("\">").append("<td><a href=\"#m")
						 * .append(scenarioIndex).append("\">") .append(
						 * methodName + "</a></td>" ) .append(
						 * "<td rowspan=\"1\">" + aResult.getName() + "</td>" )
						 * .append("<td>"+str+"</td>") .append("<td>"+aResult
	 ...
	 * += writeScenarioDetails( testResult.getFailedTestResults(),
	 * scenarioIndex); scenarioIndex += writeScenarioDetails(
	 * testResult.getSkippedConfigurationResults(), scenarioIndex);
	 * scenarioIndex += writeScenarioDetails(
	 * testResult.getSkippedTestResults(), scenarioIndex); scenarioIndex +=
	 * writeScenarioDetails( testResult.getPassedTestResults(), scenarioIndex);
	 * } } }
	 */
	/**
	 ...
	 * 
	 * ITestResult mResult = results.iterator().next(); String label =
	 * Utils.escapeHtml(className + "#" + mResult.getMethod().getMethodName() +
	 * " ( " + mResult.getName() + " )"); for (ITestResult result : results) {
	 * writeScenario(scenarioIndex, label, result); scenarioIndex++; } } }
	 * 
	 ...
	 * parameters.length); if (parameterCount > 0) { writer.print(
	 * "<tr class=\"param\">"); for (int i = 1; i <= parameterCount; i++) {
	 ...
	 * writer.print("<tr><th"); if (parameterCount > 1) { writer.print(
	 * " colspan=\""); writer.print(parameterCount); writer.print("\""); }
	 * writer.print(">Messages</th></tr>");
	 * 
	 * writer.print("<tr><td"); if (parameterCount > 1) { writer.print(
	 * " colspan=\""); writer.print(parameterCount); writer.print("\""); }
	 * writer.print(">"); writeReporterMessages(reporterMessages);
	 * writer.print("</td></tr>"); }
	 * 
	 * // Write exception (if any) Throwable throwable = result.getThrowable();
	 * if (throwable != null) { writer.print("<tr><th"); if (parameterCount > 1)
	 * { writer.print(" colspan=\""); writer.print(parameterCount);
	 * writer.print("\""); } writer.print(">"); writer.print((result.getStatus()
	 * == ITestResult.SUCCESS ? "Expected Exception" : "Exception"));
	 * writer.print("</th></tr>");
	 * 
	 * writer.print("<tr><td"); if (parameterCount > 1) { writer.print(
	 * " colspan=\""); writer.print(parameterCount); writer.print("\""); }
	 * writer.print(">"); writeStackTrace(throwable);
	 * writer.print("</td></tr>"); }
	 * 
	 * writer.print("</table>"); writer.print(
	 ...
	 * writer.print(Utils.escapeHtml(iterator.next())); while
	 * (iterator.hasNext()) { writer.print("<br/>");
	 * writer.print(Utils.escapeHtml(iterator.next())); }
	 * writer.print("</div>"); }
	 * 
	 * protected void writeStackTrace(Throwable throwable) { writer.print(
	 * "<div class=\"stacktrace\">"); writer.print(Utils.stackTrace(throwable,
	 * true)[0]); writer.print("</div>"); }
	 */
